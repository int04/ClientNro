...
if($action == "block")
{
  if($mysqli->query("SELECT * FROM `block` WHERE `uid` = '".$doituong."' AND `type` = '1'")->num_rows >=1)
  {
    echo thongbao('Người chơi này hiện tại đã bị chặn rồi.','loi');
  }
  else
  {
    $mysqli->query("INSERT INTO `block` SET `uid` = '".$doituong."', `type` = '1', `admin` = '".$id."'");
...
if($action == "unblock")
{
  if($mysqli->query("SELECT * FROM `block` WHERE `uid` = '".$doituong."' AND `type` = '1'")->num_rows <=0)
  {
    echo thongbao('Người chơi này chưa bị chặn.','loi');
  }
  else
  {
    $mysqli->query("DELETE FROM `block` WHERE `uid` = '".$doituong."' AND `type` ='1'");
...
{
    $trang = '<center>' . trang('/admin/index.php?f=block_taixiu'.$p.'&', $start, $mysqli->query("SELECT * FROM `block` $where ")->num_rows, $kmess) . '</center>';
}
$table = '';
...
<div class="col-lg-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Danh sách bị chặn tài xỉu</h4>
                    <p class="card-description"> 
                    	<form class="form-inline" action="/admin/index.php?f=block_taixiu" method="post">
